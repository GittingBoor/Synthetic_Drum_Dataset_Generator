...
def test_print_single_song_specification(builder: DatasetBuilder, index: int) -> None:
    """test-Hilfe: Erzeugt und druckt eine SongSpecification."""
    spec = builder.generate_song_specification(index)
    print("\n[test] ------------- SongSpecification -------------------")
    print(f"song_identifier : {spec.song_identifier}")
    print(f"tempo_bpm       : {spec.tempo_bpm}")
    print(f"time_signature  : {spec.time_signature}")
    print(f"number_of_bars  : {spec.number_of_bars}")
    print(f"key             : {spec.key}")
    print(f"style           : {spec.style}")
    print(f"band_config     : {spec.band_configuration}")
    print("[test] ---------------------------------------------------\n")


def test_generate_single_example(builder: DatasetBuilder) -> None:
    """test-Hilfe: Generiert ein einzelnes DatasetExample und druckt Infos."""
    spec = builder.generate_song_specification(0)
    example = builder.generate_single_example(spec)
    print("\n[test] ------------- DatasetExample ----------------------")
...
def test_build_dataset_and_save_index(builder: DatasetBuilder) -> None:
    """test-Hilfe: Baut einen kleinen Datensatz und schreibt eine Index-Datei."""
    examples = builder.build_dataset()
    print(f"[test] Anzahl erzeugter Beispiele: {len(examples)}")

    index_path = os.path.join(builder.output_root_directory, "dataset_index_test.json")
    builder.save_index(index_path)


def run_all_DatasetBuilder_tests() -> None:
    """Führt einfache Tests für den DatasetBuilder aus."""
    print("\n[TEST] =====================================================")
    print("[TEST] Starte DatasetBuilder Tests ...")

    # Für die Tests verwenden wir Fake-* Klassen, damit keine echten Audio/MIDI/Drum-Generatoren nötig sind
    output_root = "./_dataset_builder_test_output"

    fake_drum_gen = FakeDrumPatternGenerator()
    fake_harmony_gen = FakeHarmonyGenerator()
    fake_midi_builder = FakeMidiSongBuilder()
    fake_audio_renderer = FakeAudioRenderer()
    fake_label_extractor = FakeLabelExtractor()

    # Band-Konfigurationen können für diesen Test leer bleiben
    band_configuration_pool: List[BandConfiguration] = []
    ...
    builder = DatasetBuilder(
        ...
        drum_pattern_generator=fake_drum_gen,      # type: ignore[arg-type]
        harmony_generator=fake_harmony_gen,        # type: ignore[arg-type]
        midi_song_builder=fake_midi_builder,       # type: ignore[arg-type]
        audio_renderer=fake_audio_renderer,        # type: ignore[arg-type]
        label_extractor=fake_label_extractor,      # type: ignore[arg-type]
        random_seed=1234,
    )

    print("\n[TEST] --- Einzelne SongSpecification ---")
    test_print_single_song_specification(builder, index=0)

    print("\n[TEST] --- Ein einzelnes DatasetExample ---")
    test_generate_single_example(builder)

    print("\n[TEST] --- Kompletten kleinen Datensatz bauen + Index ---")
    test_build_dataset_and_save_index(builder)

    print("\n[TEST] DatasetBuilder Tests abgeschlossen.")
    print("[TEST] =====================================================\n")


if __name__ == "__main__":
    run_all_DatasetBuilder_tests()
